...
estimate_linear_lp <- function(df,
  ...
  if (std == "nw") {
    ...
    for (h in 0:horizons) {
      ...
      #   .x = ifelse(
      #     add_intercept,
      #     yes = group_names_full[-length(group_names_full)],
      #     no = group_names_full
      #   ),
      #   .f = function(group_idx) {
      #     j <- X[, group_idx] != 0
      #     X_j <- X[j, , drop = FALSE]
      ...
      # )
      # # no. clusters
      # n_cl <- length(group_names_full)
      # if (add_intercept == TRUE) n_cl <- n_cl-1
      # omega <- (n - 1) / (n - p) * (n_cl / (n_cl - 1)) * Reduce('+', omegaj)
      # Sandwich
      # cl_std[[h + 1]] <- sqrt(diag(crossXinv %*% omega %*% crossXinv))
      # names(cl_std[[h + 1]]) <- colnames(omega)
    }
    se_df <-
